...
> [!TIP]
> You can install a specific version using -x.y.z, e.g. elasticsearch-drift-plugin-8.19.2
...
```bash
 make ES_VERSION=8.19.2 PACKAGE_VERSION=0 packages
...
```
/usr/share/elasticsearch/bin/elasticsearch-plugin install file:///path/to/elasticsearch-drift/plugin/target/releases/elasticsearch-drift-plugin-8.19.2-2.2.0.zip
```
