...
> [!TIP]
> You can install a specific version using -x.y.z, e.g. elasticsearch-drift-plugin-9.0.1
...
```bash
 make ES_VERSION=9.0.1 PACKAGE_VERSION=0 packages
...
```
/usr/share/elasticsearch/bin/elasticsearch-plugin install file:///path/to/elasticsearch-drift/plugin/target/releases/elasticsearch-drift-plugin-9.0.1-2.2.1.zip
```
